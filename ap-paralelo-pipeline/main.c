...
int main(int argc, char *argv[])
{
	...
	{
		thread_args *thread_data = create_thread_args(i, base_path, pipe_r, watermark, pipe_threads);
	...
	int thread_to_close;
	int i=0;
	while ((i!=max_threads) && (read(pipe_threads[0], &thread_to_close, sizeof(int)) == sizeof(int)))
	{
		i++;
